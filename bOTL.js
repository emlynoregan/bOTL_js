...
(function(root, factory) {
    ...
    function _transform(aScope, abOTLTransform) 
    {
    ...
    function GetSectionFunction(abOTLTransform) 
    {
        ...
        else if (isString(abOTLTransform)) 
        {
            if (abOTLTransform.slice(0, 1) == '#')
                lfunction = function(aScope) {
                    return EvaluateFullSection(aScope, {'_type': '#','path': abOTLTransform.slice(1)})
                };
            else
                lfunction = function(aScope) {
    ...
    function EvaluateObjectSection(aScope, abOTLObjectSection) 
    {
        ...
        if (isObject(lvalue)) 
        {
            ...
            for (var lkey in lvalue) 
            {
                ...
                {
                    var lchildResults = _transform(aScope, lvalue[lkey]);
    ...
    function EvaluateObject(aScope, abOTLObject) 
    {
        ...
        {
            var ltransformedValue = _transform(aScope, abOTLObject[lkey]);
